...
for file in train_dirs:
        ...
        with open(file + '/id_timestamps.txt') as f:
            ...
            topic_id = np.array([int(line[0]) for line in lines])-50
...
for j in range(max(topic_id)):
    ...
    q_sub[j]=(np.full((len(q[j])), querys_timestamps[j+50], dtype=int)) - q[j]
    xmax = max(q_sub[j])
    xmin = min(q_sub[j])
    ...
    kde_eval[j] = kde.evaluate(x_grid)*10**8
