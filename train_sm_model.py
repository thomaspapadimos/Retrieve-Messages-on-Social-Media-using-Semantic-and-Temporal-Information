...
from text_preprocessing import create_vocab, load_embeddings_from_file
...
def create_train_data(train_dirs, include_extra_features=True):
    ...
    return q_train, d_train, y_train, addit_feat_train
...
def create_test_data(test_dir, include_extra_features=True):
    ...
    return q_test, d_test, y_test, addit_feat_test, qids_test, ql_pred
...
def train_model():
    ...
    '''
    for file in ['trec-2011', 'trec-2012', 'trec-2013', 'trec-2014']:
        with open(dir + file + '/a.toks') as f:
            a_seq = get_sequences(tokenizer, [line for line in f])
            with open(dir + file + '/a.seq', 'w') as o_f:
                wr = csv.writer(o_f)
                wr.writerows(a_seq)

        with open(dir + file + '/b.toks') as f:
            b_seq = get_sequences(tokenizer, [line for line in f])
            with open(dir + file + '/b.seq', 'w') as o_f:
                wr = csv.writer(o_f)
                wr.writerows(b_seq)
    '''
    ...
    for test_dir in all_dirs:
        ...
        # Get train data
        q_train, d_train, y_train, addit_feat_train = create_train_data(train_dirs, True)
        addit_feat_len = addit_feat_train.shape[1] if addit_feat_train.ndim > 1 else 1

        # Get test data and baseline ranking
        q_test, d_test, y_test, addit_feat_test, qids_test, ql_pred = create_test_data(test_dir, True)
        ...
        q_train, a_train, y_train, addit_feat_train = shuffle(q_train,
                                                              d_train,
                                                              y_train,
                                                              addit_feat_train,
        ...
        for epoch in range(config.EPOCHS):
            ...
            batch_no = 0
            test_metrics = []
            for q_train_batch, d_train_batch, y_train_batch, addit_feat_train_batch in batches:
                sm_model.train_on_batch([q_train_batch, d_train_batch, addit_feat_train_batch], y_train_batch)
                batch_metrics = sm_model.test_on_batch([q_train_batch, d_train_batch, addit_feat_train_batch], y_train_batch)
            ...
            y_pred = sm_model.predict([q_test, d_test, addit_feat_test])
